...
class SystemHealthMonitorApp:
    ...
    def create_system_info_panel(self):
        ...
        info_text = f"🖥️ OS: {system_info['os']} | 💻 CPU: {system_info['cpu']}"
    ...
    def blink_loop(self):
        ...
        # Re-run update_health_score to apply the blink effect if needed
        if hasattr(self, 'current_score'):
            # Recalculate status to ensure we have the latest info
            _, score_status = self.health_calculator.get_score_status(
                self.current_score)
